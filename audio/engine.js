...
// Construction paresseuse du graphe audio: aucun node Tone n'est créé à
// l'import du module. Chaque appel retourne un graphe indépendant complet
// (bus, effets parallèles, compression, limiter, meter) avec ses méthodes
// de contrôle.
export function createAudioGraph() {
  // Headroom global (gain en dB converti plus tard si besoin), limiter pour éviter clipping
  // Headroom initial à -10 dB pour réduire risques de pompage/écrêtage
  ...
  const reverbHP = new Tone.Filter({
    type: "highpass",
    frequency: 220,
    Q: 0.7,
  });
  const reverbLP = new Tone.Filter({
    type: "lowpass",
    frequency: 4800,
    Q: 0.4,
  });
  ...
  const longRevHP = new Tone.Filter({
    type: "highpass",
    frequency: 180,
    Q: 0.6,
  });
  ...
  // Les effets reçoivent des signaux des synths (qui se connectent sur leur
  // entrée) et renvoient leur wet vers bus via leurs gains respectifs.
  ...
  function setReverbDepth(factor = 1) {
    ...
    wet.reverb.gain.rampTo(0.78 * wetScale, 0.35);
    wet.longReverb.gain.rampTo(0.74 * wetScale, 0.45);
  ...
  function enableAutoHeadroom(
    ...
    if (enable && !headroomGuard.enabled) {
      ...
      if (headroomGuard.schedId != null) transport.clear(headroomGuard.schedId);
